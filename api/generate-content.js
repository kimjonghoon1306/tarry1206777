...
export default function ContentGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      clearInterval(interval);
      if (e.isWarning && e.content) {
        // 글은 있지만 불완전한 경우 - 글 넣고 경고만 표시
        const tags = generateHashtags(keyword, title, e.content);
        setHashtags(tags);
        setGeneratedContent(e.content);
        setCharCount(e.content.length);
        setProgress(100);
        toast.warning(e.message, { id: "generate", duration: 6000 });
      } else {
