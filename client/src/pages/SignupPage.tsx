...
import { Bot, Eye, EyeOff, CheckCircle2, XCircle, ArrowLeft } from "lucide-react";
...
function Rule({ ok, text }: { ok: boolean; text: string }) {
  return (
    <div className="flex items-center gap-2 text-xs">
      {ok
        ? <CheckCircle2 className="w-3.5 h-3.5 shrink-0" style={{ color: "var(--color-emerald)" }} />
        : <XCircle className="w-3.5 h-3.5 shrink-0" style={{ color: "var(--muted-foreground)" }} />}
...
export default function SignupPage() {
  ...
  const set = (k: string) => (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (k === "email" && val) {
      setErrors(prev => ({
        ...prev,
        email: !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val) ? "올바른 이메일 형식이 아니에요" : ""
      }));
    } else if (k === "confirm" && val) {
      setErrors(prev => ({ ...prev, confirm: val !== form.password ? "비밀번호가 일치하지 않아요" : "" }));
    } else {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4" style={{ background: "var(--background)" }}>
      ...
      <div className="w-full max-w-sm space-y-5">
          ...
          <div className="space-y-2">
            <div className="relative">
              <Input type={showPw ? "text" : "password"} placeholder="비밀번호"
                value={form.password} onChange={set("password")} className="h-12 text-base pr-12"
        ...
        <p className="text-center text-sm pb-6" style={{ color: "var(--muted-foreground)" }}>
          이미 계정이 있으신가요?{" "}
