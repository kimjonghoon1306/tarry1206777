...
const FALLBACK = [
  { id:"f1",  title:"청담 오마카세 디너 체험단",    source:"강남맛집체험단", region:"서울", tags:["방문형","블로그","파인다이닝"], reward:"18만원 상당", deadline:2,  rewardVal:180000, url:"https://강남맛집.net"         },
  { id:"f2",  title:"압구정 이탈리안 레스토랑 런치", source:"강남맛집체험단", region:"서울", tags:["방문형","인스타","맛집"],      reward:"8만원 상당",  deadline:1,  rewardVal:80000,  url:"https://강남맛집.net"         },
  { id:"f3",  title:"강남 루프탑 바 디너 체험",      source:"디너의여왕",    region:"서울", tags:["방문형","블로그","다이닝"],     reward:"12만원 상당", deadline:3,  rewardVal:120000, url:"https://dinnerqueen.net"      },
  { id:"f4",  title:"신사 파스타 맛집 저녁 체험",    source:"디너의여왕",    region:"서울", tags:["방문형","인스타","맛집"],      reward:"6만원 상당",  deadline:5,  rewardVal:60000,  url:"https://dinnerqueen.net"      },
  { id:"f5",  title:"홍대 카페 신메뉴 음료 체험단",  source:"파블로",        region:"서울", tags:["방문형","블로그","카페"],       reward:"2만원 상당",  deadline:4,  rewardVal:20000,  url:"https://pavlovu.com"          },
  { id:"f6",  title:"판교 샐러드 전문점 체험",       source:"파블로",        region:"경기", tags:["방문형","블로그","건강식"],     reward:"3만원 상당",  deadline:7,  rewardVal:30000,  url:"https://pavlovu.com"          },
  { id:"f7",  title:"수원 고기집 저녁 체험단",       source:"모두의체험단",  region:"경기", tags:["방문형","블로그","맛집"],      reward:"4만원 상당",  deadline:6,  rewardVal:40000,  url:"https://www.modan.kr"         },
  { id:"f8",  title:"강릉 오션뷰 카페 베이커리",     source:"파블로",        region:"강원", tags:["방문형","인스타","카페"],       reward:"3만원 상당",  deadline:9,  rewardVal:30000,  url:"https://pavlovu.com"          },
  { id:"f9",  title:"속초 해산물 맛집 2인 코스",     source:"태그바이",      region:"강원", tags:["방문형","블로그","맛집"],      reward:"6만원 상당",  deadline:11, rewardVal:60000,  url:"https://www.tagby.io/recruit" },
  { id:"f10", title:"해운대 스파 & 마사지 체험",     source:"태그바이",      region:"부산", tags:["방문형","인스타","뷰티"],      reward:"12만원 상당", deadline:3,  rewardVal:120000, url:"https://www.tagby.io/recruit" },
  { id:"f11", title:"홈케어 스킨케어 세트 배송",     source:"모두의체험단",  region:"전국", tags:["배송형","블로그","뷰티"],      reward:"상품 제공",   deadline:14, rewardVal:50000,  url:"https://www.modan.kr"         },
  { id:"f12", title:"건강식 도시락 구독 체험",       source:"파블로",        region:"전국", tags:["배송형","유튜브","푸드"],       reward:"15만원 상당", deadline:2,  rewardVal:150000, url:"https://pavlovu.com"          },
];
...
.ch-card { animation:fadeUp 0.4s ease both; transition:transform 0.2s,box-shadow 0.2s; }
.ch-card:hover { transform:translateY(-3px); box-shadow:0 8px 24px rgba(0,0,0,0.22); }
.ch-hot  { animation:pinkGlow 3s ease-in-out infinite, fadeUp 0.4s ease both; transition:transform 0.2s,box-shadow 0.2s; }
...
.ch-apply{ transition:all 0.18s; } .ch-apply:hover { transform:scale(1.05); opacity:0.9; }
`;
...
export default function CampaignPage() {
  ...
  return (
    <Layout>
      <style>{CSS}</style>
      <div style={{ padding:"28px 28px 48px", opacity:ready?1:0, transform:ready?"none":"translateY(10px)", transition:"opacity 0.5s,transform 0.5s" }}>
        ...
        <div style={{ marginBottom:"24px" }}>
          ...
          <p style={{ fontSize:"13px", color:"var(--muted-foreground)", margin:0 }}>주요 체험단 사이트의 모집 공고를 한 곳에서 확인하세요 · 신청은 해당 사이트에서</p>
          {updatedAt && <p style={{ fontSize:"11px", color:"var(--muted-foreground)", margin:"4px 0 0", opacity:0.6 }}>마지막 업데이트: {fmtDate(updatedAt)} · 45분마다 자동 갱신</p>}
        </div>
            ...
            {SOURCES.map(s => {
              ...
              return (
                <div key={s.name} className="ch-src" onClick={() => window.open(s.url, "_blank")}
                  ...
                  <span style={{ fontSize:"11px", color:hot?"#f472b6":"var(--color-emerald)" }}>{sourceCounts[s.name] || "-"}</span>
                </div>
              );
            })}
          </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"12px", marginBottom:"20px" }}>
          {[
            { label:"전체 캠페인", val:list.length,                          color:"var(--color-emerald)", sub:"수집된 공고"  },
            { label:"마감 임박",   val:list.filter(c=>c.deadline<=3).length,  color:"#f97316",              sub:"3일 이내"    },
            { label:"추천 사이트", val:"강남맛집 · 디너의여왕",                color:"#f472b6",              sub:"선정률 높음" },
          ].map((s, i) => (
          ...
          <div style={{ display:"flex", gap:"6px", flexWrap:"wrap" }}>
            {REGIONS.map(r => (
              <button key={r} className="ch-rtab" onClick={() => setRegion(r)}
                style={{ background:r===region?"linear-gradient(90deg,#be185d,#ec4899)":"transparent", border:`1px solid ${r===region?"#ec4899":"var(--border)"}`, color:r===region?"#fff":"var(--muted-foreground)", fontSize:"12px", padding:"6px 14px", borderRadius:"20px", cursor:"pointer", fontWeight:r===region?600:400 }}>
                {r}
              </button>
            ))}
          </div>
          <select value={sort} onChange={e => setSort(e.target.value)}
            style={{ background:"var(--muted)", border:"1px solid var(--border)", color:"var(--foreground)", fontSize:"12px", padding:"6px 12px", borderRadius:"8px", cursor:"pointer" }}>
        ...
        <div style={{ fontSize:"11px", color:"var(--muted-foreground)", textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:"14px" }}>
          ...
          {loading && <span style={{ marginLeft:"8px", opacity:0.5 }}>불러오는 중...</span>}
        </div>
          ...
          {list.map((c, idx) => {
            ...
            return (
              <div key={String(c.id)} className={hot ? "ch-hot" : "ch-card"}
                style={{ background:"var(--muted)", border:`1px solid ${hot?"rgba(236,72,153,0.35)":"var(--border)"}`, borderRadius:"12px", padding:"16px", animationDelay:`${idx*0.04}s`, position:"relative", overflow:"hidden" }}>
                {/* 상단 컬러 바 */}
                ...
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"10px", marginTop:"4px" }}>
                  <span style={{ fontSize:"10px", padding:"2px 8px", borderRadius:"4px", background:hot?"rgba(236,72,153,0.12)":"var(--background)", color:hot?"#f472b6":"var(--muted-foreground)", border:`1px solid ${hot?"rgba(236,72,153,0.3)":"var(--border)"}` }}>
                    {c.source}{hot ? " ✓" : ""}
                  </span>
                  <span style={{ fontSize:"10px", padding:"2px 8px", borderRadius:"4px", background:"rgba(16,185,129,0.1)", color:"var(--color-emerald)", border:"1px solid rgba(16,185,129,0.2)", display:"flex", alignItems:"center", gap:"3px" }}>
                    <MapPin style={{ width:9, height:9 }} />{c.region}
                  </span>
                </div>
                {/* 제목 */}
                ...
                {c.tags?.length > 0 && (
                  <div style={{ display:"flex", gap:"5px", flexWrap:"wrap", marginBottom:"12px" }}>
                    {c.tags.map((t: string) => (
                      <span key={t} style={{ fontSize:"10px", padding:"2px 7px", borderRadius:"4px", background:"var(--background)", color:"var(--muted-foreground)", border:"1px solid var(--border)" }}>{t}</span>
                    ))}
                  </div>
                )}
                {/* 하단 */}
