...
// ── 이미지 URL 사전 판정 제거 ──────────────────────────
// 실제 성공/실패 판정은 각 카드의 <img onLoad / onError> 에서 처리한다.
// 여기서 미리 테스트하면 느린 응답까지 실패로 오판정해서 정상 이미지가 실패로 찍힐 수 있다.
...
// ── 이미지 생성: URL만 생성하고 실제 성공/실패는 카드에서 판정 ──
async function generatePollinationsUrl(
  ...
  return `https://image.pollinations.ai/prompt/${encoded}?width=${width}&height=${height}&nologo=true&seed=${seed}&model=flux&t=${ts + seed}`;
}
