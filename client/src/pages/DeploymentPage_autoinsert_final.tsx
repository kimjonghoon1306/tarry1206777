...
export default function DeploymentPage() {
  ...
  const buildAutoImageBlocks = (imagesInput: { id?: number; src: string; alt?: string }[], textCount: number) => {
    const images = imagesInput.filter((img) => typeof img?.src === "string" && img.src).slice(0, 15);
    if (images.length === 0 || textCount === 0) return [];

    const groupCount = Math.min(textCount, Math.max(1, (images.length + 2) // 3));
    const base = images.length // groupCount;
    const remainder = images.length % groupCount;
    const sizes = Array.from({ length: groupCount }, (_, i) => base + (i < remainder ? 1 : 0));

    const grouped: Extract<ContentBlock, { type: "image" }>[][] = [];
    let cursor = 0;
    for (let i = 0; i < sizes.length; i++) {
      const size = sizes[i];
      const group: Extract<ContentBlock, { type: "image" }>[] = [];
      for (let j = 0; j < size && cursor < images.length; j++) {
        group.push({
          type: "image",
          id: uid(),
          src: images[cursor].src,
          alt: images[cursor].alt || `이미지 ${cursor + 1}`,
          position: "center",
          source: "auto",
        });
        cursor += 1;
  ...
  const autoInsertImages = (imagesInput?: { id?: number; src: string; alt?: string }[]) => {
    const sourceImages = (imagesInput || deployImages).filter((img) => img?.src);
    if (sourceImages.length === 0) {
      toast.error("이미지 생성 페이지에서 이미지를 먼저 선택해주세요");
      return;
    }
    ...
    const imageGroups = buildAutoImageBlocks(sourceImages, textBlocks.length);
    const result: ContentBlock[] = [];
    const interval = Math.max(1, Math.floor(textBlocks.length / imageGroups.length));
    let textIndex = 0;
    let groupIndex = 0;

    while (textIndex < textBlocks.length) {
      result.push(textBlocks[textIndex]);

      const shouldInsert = groupIndex < imageGroups.length && (
        textIndex === textBlocks.length - 1 ||
        textIndex % interval === interval - 1 ||
        result.length === 1
      );

      if (shouldInsert) {
        result.push(...imageGroups[groupIndex]);
        groupIndex += 1;
      }
    ...
    while (groupIndex < imageGroups.length) {
      result.push(...imageGroups[groupIndex]);
      groupIndex += 1;
    }

    if (manualImages.length > 0) result.push(...manualImages);

    if (!thumbnail && sourceImages[0]?.src) {
      setThumbnail(sourceImages[0].src);
    }

    setBlocks(result);
    setAutoInserted(true);
    toast.success(`최대 15장 기준으로 ${Math.min(sourceImages.length, 15)}장의 이미지가 자동 삽입되었습니다`);
  };
          ...
          <div className="lg:col-span-2 space-y-3 sm:space-y-4">
            ...
            <div className="rounded-xl p-4" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div>
                  ...
                  <p className="text-xs" style={{ color: "var(--muted-foreground)" }}>
                    이미지 생성 페이지에서 선택한 이미지 중 최대 15장을 본문 중간에 2~3장씩 자동 배치합니다
                  </p>
                </div>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      try {
                        ...
                        toast.success(`선택 이미지 ${Math.min(list.length, 15)}장을 불러왔어요`);
                      } catch {
                    ...
                    {deployImages.slice(0, 15).map((img, i) => (
                      <button
                        ...
                        style={{
                          width: 64,
                          height: 64,
                          border: thumbnail === img.src ? "2px solid var(--color-emerald)" : "1px solid var(--border)",
                        }}
                        onClick={() => {
              ...
              {autoInserted && (
                ...
                >
                  자동 삽입이 적용되었습니다. 최대 15장만 사용되며, 본문 구간마다 2~3장씩 분배됩니다.
                </div>
              )}
            </div>
