...
export default function KeywordResearch() {
  ...
  const handleCollect = async () => {
    ...
    try {
      const seedKeywords = ["맛집", "여행", "재테크", "다이어트", "인테리어", "강아지"];
      const resp = await fetch("/api/naver-keywords", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ accessLicense, secretKey, customerId, keywords: seedKeywords }),
      });
      ...
      if (list.length === 0) throw new Error("수집된 키워드가 없습니다");

      const newKeywords = list.slice(0, 10).map((item: any, i: number) => {
        const pc = parseInt(item.monthlyPcQcCnt?.replace?.(",", "") || "0") || 0;
        const mobile = parseInt(item.monthlyMobileQcCnt?.replace?.(",", "") || "0") || 0;
        const total = pc + mobile;
        const comp = item.compIdx === "높음" ? "높음" : item.compIdx === "낮음" ? "낮음" : "중";
        return {
          id: Date.now() + i,
      ...
      toast.success(`네이버 키워드 ${newKeywords.length}개 수집 완료!`, { id: "collect" });
    } catch (e: any) {
