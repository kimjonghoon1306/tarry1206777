...
- 오직 한글, 영어, 숫자만 사용${styleGuide}`;
  ...
  try {
    ...
    // ── Gemini (자동 재시도 최대 3회, 429시 15초 대기) ────────
    if (provider === "gemini") {
      // gemini-2.0-flash: 무료 한도 넉넉 (RPM 15, 일 1500)
      // gemini-2.5-flash: RPM이 더 낮아 자주 429 발생 → 2.0으로 통일
      const GEMINI_MODEL = "gemini-2.0-flash";
      const MAX_RETRIES = 3;
      const RETRY_DELAY_MS = 15000; // 15초 대기 (분당 제한 해소)
      ...
      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        ...
        if (!resp.ok) {
          ...
          // API 키 오류는 재시도 의미 없음 → 즉시 throw
          if (msg.includes("api key") || msg.includes("api_key") || status === 403) {
            throw new Error("Gemini API 키가 잘못되었거나 권한이 없습니다. 설정에서 확인해주세요.");
          }

          // 429 분당 한도 초과 → 재시도
          if (status === 429 || msg.includes("quota") || msg.includes("resource_exhausted") || msg.includes("rate")) {
            lastError = `분당 요청 한도 초과 (${attempt}/${MAX_RETRIES}회 시도)`;
            if (attempt < MAX_RETRIES) {
              await new Promise(r => setTimeout(r, RETRY_DELAY_MS));
              continue;
            }
            throw new Error(`Gemini 분당 한도 초과로 ${MAX_RETRIES}회 재시도했지만 실패했어요. 잠시 후 다시 시도하거나 Groq(무료)으로 전환해보세요.`);
          }
        ...
        if (!text) {
          ...
          throw new Error("Gemini 응답이 비어있습니다. 잠시 후 다시 시도해주세요.");
        }
        return res.json({ content: removeNonKorean(text) });
      }
      throw new Error(lastError || "Gemini 오류");
    }
