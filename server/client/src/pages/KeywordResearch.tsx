...
export default function KeywordResearch() {
  ...
  const [inputKW, setInputKW] = useState(""); // 검색창 입력값
  ...
  const [collectCount, setCollectCount] = useState(0); // 강제 재실행 트리거
  ...
  function cycleSort() {
    setSort(s => s==="volume"?"hard":s==="hard"?"easy":"volume");
  }
  ...
  async function doCollect(query?: string) {
    ...
    setCollectCount(c => c+1); // 강제 재실행 카운터
    ...
    try {
      ...
      toast.success(`키워드 ${newKWs.length}개 수집 완료! (총 ${keywords.length + newKWs.length}개)`, { id:"collect" });
    } catch (e: any) {
      ...
      <div className="p-4 sm:p-6 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs mt-0.5" style={{color:"var(--muted-foreground)"}}>
              수집 {collectCount}회 · 총 {keywords.length}개
            </p>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" size="sm" className="gap-1.5" onClick={()=>toast.info("준비 중")}>
        ...
        <div className="rounded-xl" style={{background:"var(--card)",border:"1px solid var(--border)"}}>
          ...
          <div className="flex items-center gap-2 p-3 sm:p-4 border-b" style={{borderColor:"var(--border)"}}>
            <div className="relative flex-1">
              ...
              <Input
                placeholder="키워드 입력 후 Enter 또는 수집 버튼"
          ...
          {/* 테이블 헤더 (데스크탑) */}
          <div className="hidden sm:grid grid-cols-12 gap-2 px-4 py-2 text-xs font-semibold uppercase tracking-wider"
          ...
          <div className="divide-y" style={{borderColor:"var(--border)"}}>
            {sorted.map(kw => {
              const cs = compStyle(kw.competition);
              return (
                  ...
                  <div className="flex items-center gap-2 sm:hidden">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <span className="text-xs px-1.5 py-0.5 rounded" style={{background:cs.bg,color:cs.fg}}>{kw.competition}</span>
                      </div>
                    </div>
                  ...
                  <div className="hidden sm:grid grid-cols-12 gap-2 items-center">
                    <div className="col-span-4 flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <span className="text-xs px-1.5 py-0.5 rounded" style={{background:cs.bg,color:cs.fg}}>경쟁 {kw.competition}</span>
                      </div>
                    </div>
        ...
        {selKW && (
          <div className="rounded-xl p-4 sm:p-5"
            style={{background:"var(--card)",border:"2px solid oklch(0.75 0.12 300/50%)"}}>
