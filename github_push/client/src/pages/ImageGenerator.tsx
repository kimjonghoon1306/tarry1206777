...
// ── 이미지 생성: Pollinations URL 즉시 반환 ──────────────
function generatePollinationsUrl(
  prompt: string, width: number, height: number, seed: number
): string {
  const encoded = encodeURIComponent(prompt);
  // flux 모델 불안정 → 기본 모델 사용, enhance=false로 안정성 확보
  return `https://image.pollinations.ai/prompt/${encoded}?width=${width}&height=${height}&nologo=true&seed=${seed}&enhance=false&safe=false`;
}
...
if (provider === "pollinations") {
  ...
  const handleRetryAll = async () => {
    ...
    // 새 seed로 URL 재생성 후 즉시 적용 (await 불필요 - sync 함수)
    setGallery(prev => prev.map(item => {
      const failedItem = failed.find(f => f.id === item.id);
      if (!failedItem) return item;
      const seed = Math.floor(Math.random() * 999999) + Date.now() + item.id;
    ...
    setProgress(100);
    toast.success(`✅ ${failed.length}개 재시도 완료! 이미지가 로드됩니다.`, { id: "imggen" });
    setIsRetrying(false);
  };
